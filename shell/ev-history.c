...
ev_history_get_current_page (EvHistory *history)
{
	...
	switch (ev_link_dest_get_dest_type (dest))
	{
		...
		case EV_LINK_DEST_TYPE_PAGE_LABEL:
		{
			...
			document = ev_document_model_get_document (history->priv->model);
			ev_document_find_page_by_label (document, ev_link_dest_get_page_label (dest), &page);
                      ...
                      EvDocumentModel *model)
{
	...
	{
		g_object_remove_weak_pointer (G_OBJECT (history->priv->model), (gpointer)&history->priv->model);

		if (history->priv->page_changed_handler_id)
		{
			g_signal_handler_disconnect (history->priv->model, history->priv->page_changed_handler_id);
	...
	g_object_add_weak_pointer (G_OBJECT (model), (gpointer)&history->priv->model);

	g_signal_connect (history->priv->model, "notify::document",
					  G_CALLBACK (document_changed_cb), history);
	history->priv->page_changed_handler_id =
		g_signal_connect (history->priv->model, "page-changed",
						  G_CALLBACK (page_changed_cb), history);
}
