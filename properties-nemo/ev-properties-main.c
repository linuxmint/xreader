...
static GList *ev_properties_get_pages
	(NemoPropertyPageProvider *provider, GList *files);
...
static GList *
ev_properties_get_pages (NemoPropertyPageProvider *provider,
			 GList *files)
{
	GError *error = NULL;
	EvDocument *document;
	...
	property_page = nemo_property_page_new ("document-properties",
			label, page);
